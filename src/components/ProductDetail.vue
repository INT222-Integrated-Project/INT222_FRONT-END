        ...
        <div class="w-10/12  ">
          <div class="card-two flex justify-center item-center ">
            <form @submit.prevent="addtocard">
                  ...
                  <div class="flex-1  ">
                    ...
                    <div class="flex flex-row items-center justify-center    ">
                      <select :value="c" id="productColor" name="productColor" v-model="productCart.productColorId"
                        class="bg-gray-100 rounded py-2 mb-4 w-full h-10 flex mx-2 items-center text-center ">
                        <option disabled value="Colors">Colors</option>
                        <option v-for="c in product.productColor" :key="c" selected
                          :class=" c.color.caseColor ? 'bg-caseCol-' + c.color.caseColor.toLowerCase(): '' ? 'text-caseCol-'+ c.color.caseColor.toLowerCase(): ''  ">
                          {{c.productcolorID}}</option>
                      </select>
                    </div>
              ...
              <div v-show="this.orderNotification.successOrder"
                class="mx-2 default-success-notification-window flex content-center justify-center">
                <div class="">successfully ordered!.<br />Check it out at your profile!</div>
                <button type="button" class="defaultinput-button-small-green"
                  @click="closeOrderSuccessNotification">OK</button>
              </div>
            </form>
